...
| `burn_subtitles_codec` | 视频编码器，可选 `libx264` / `libx265` / `h264_nvenc` | `libx264` |
| `burn_subtitles_crf` | 质量值（软编码为 CRF，NVENC 为 CQ），取值 0-51，越小画质越好 | `18` |
...
- **h264_nvenc**：NVIDIA 硬件编码。编码速度快、CPU 占用低，需要满足以下条件：
  - 使用 NVIDIA 显卡且已安装显卡驱动；
  - 当前 FFmpeg 构建包含 `h264_nvenc` 编码器（可通过 `ffmpeg -encoders | grep nvenc` 验证）；
  - 显卡支持 NVENC（GTX 10 系列及以上基本都支持）。

## 质量参数（CRF / CQ）

质量值与编码器联动：

- 使用 **libx264 / libx265** 时，质量值对应 FFmpeg 的 `-crf` 参数；
- 使用 **h264_nvenc** 时，NVENC 编码器不支持 CRF，后端自动改传恒定质量参数
  `-rc:v vbr -cq <值> -b:v 0 -preset <档位>`（`cq` 为 NVENC 的恒定质量参数，`-b:v 0` 表示
  码率由质量决定、不设上限）。

两者取值均为 0-51，越小画质越好、文件越大，默认 18。
...
| h264_nvenc | `p1` → `p7`（p1 最快、p7 画质最好，常用 p5） | `p5` |
...
- 指定 `h264_nvenc` 时请确认 FFmpeg 支持该编码器，否则烧录阶段会报错。